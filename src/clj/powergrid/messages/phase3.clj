    ...
    (let [player (g/player game player-id)]
      (cond
        (or (not (map? resources))
            (empty? resources)) (fail "Invalid resources")
        (not (every? r/types (keys resources)))
        (fail "Unknown resource(s) specified")

        (some neg? (vals resources)) (fail "Invalid resource amount")
        (not (p/has-capacity? player resources)) (fail "Insufficient power-plant capacity")
        (not (g/contains-resources? game resources)) (fail "Insufficient resources in market")
        (not (p/can-afford? player (total-price game resources))) (fail "Insufficient funds")
        :else game)))
